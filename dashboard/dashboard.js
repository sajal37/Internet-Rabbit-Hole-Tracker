...
function getOverviewInsights(session, state) {
  ...
  const cached = app.cache.overviewInsights.get(session.id);
  ...
  app.cache.overviewInsights.set(session.id, {
    key: insightKey,
    data: allInsights,
  });
  return allInsights;
...
function scheduleFrameRender(key, renderFn) {
  ...
  app.cache.realtimeFrameQueue.set(key, renderFn);
...
function getSessionCacheKey(session) {
  ...
  return `${session.id}:${session.updatedAt || 0}:${nodeCount}:${edgeCount}:${getInsightSettingsKey()}`;
}
...
function getDerivedSessionData(session) {
  ...
  const key = getSessionCacheKey(session);
  const cached = app.cache.sessionDerived.get(session.id);
  ...
  const data = {
    ...
    damageReceipts: buildDamageReceipts(session),
    deepestChain: computeDeepestChain(session),
  };
  app.cache.sessionDerived.set(session.id, { key, data });
  return data;
}
...
function ensureSessionInsights(session) {
  ...
  const insightKey = `${session.updatedAt || 0}:${Object.keys(
    session.nodes || {},
  ).length}:${getInsightSettingsKey()}`;
  ...
  if (session._insightsKey === insightKey && session.categoryTotals && hasIntentDrift) {
    return;
  }
  const nodes = Object.values(session.nodes || {});
  if (!nodes.length) {
    session.label = session.label || "No activity captured.";
    session.labelDetail = session.labelDetail || "";
    session.categoryTotals = session.categoryTotals || {};
    session.distractionAverage = session.distractionAverage || 0;
    session.distractionNormalized =
      session.distractionNormalized ??
      normalizeDistractionScore(session.distractionAverage);
    session.distractionLabel =
      session.distractionLabel ||
      getDistractionLabel(session.distractionNormalized);
    applyIntentDrift(session, computeSessionSignals(session));
    session._insightsKey = insightKey;
    return;
  }
  ...
  nodes.forEach((node) => {
    ...
    weightedScore += activeMs * (node.distractionScore || 0);
...
function computeDistractionScore(node, session, signals = null) {
...
function pickEarlyCategory(nodes, session) {
  ...
  const baseline =
    session.firstActivityAt || session.startedAt || sorted[0].firstSeen || Date.now();
...
function buildTopDomains(session) {
  ...
  return Array.from(domainMap.values()).sort((a, b) => b.activeMs - a.activeMs);
}
...
function buildDamageReceipts(session) {
  ...
  if (shortsMs >= 5 * 60 * 1000) {
    receipts.push(
      `Short-form content held attention for ${formatDuration(shortsMs)}.`,
    );
  } else if (session.categoryTotals) {
    const dominant = pickDominantCategory(session.categoryTotals);
    if (dominant) {
      const duration = session.categoryTotals[dominant] || 0;
      const lower = dominant.toLowerCase();
      receipts.push(
        `Most time went to ${lower} content (${formatDuration(duration)}).`,
      );
    }
  }
  ...
  const trap = (session.trapDoors || [])[0];
  if (trap) {
    const domain = getDomain(trap.url) || truncate(trap.url, 36);
    receipts.push(`Turning point: ${domain}.`);
  }

  const topDomain = buildTopDomains(session)[0];
  if (topDomain) {
    receipts.push(
      `Largest pull: ${topDomain.domain} (${formatDuration(topDomain.activeMs)}).`,
    );
  }
...
function findSessionEndUrl(session) {
  ...
  nodes.sort((a, b) => (a.lastSeen || 0) - (b.lastSeen || 0));
  return nodes[nodes.length - 1].url;
}
...
function formatPathNode(url) {
  ...
  return getDomain(url) || truncate(url, 28);
}

function buildCalloutMessages(session, state, tone) {
  const mode = tone === "direct" ? "direct" : "neutral";
  const messages = [];
  const trap = (session.trapDoors || [])[0];
  if (trap) {
    const domain = getDomain(trap.url) || truncate(trap.url, 36);
    messages.push(
      mode === "direct"
        ? `Turning point again: ${domain}.`
        : `Turning point again: ${domain}.`,
    );
  }

  if (app.settings?.intentDriftAlerts && session.intentDriftLabel === "High") {
    const reason = session.intentDriftReason || "Intent drift detected.";
    const drivers = Array.isArray(session.intentDriftDrivers)
      ? session.intentDriftDrivers
      : [];
    const driverText = drivers.length ? ` ${drivers.join(" ")}` : "";
    messages.push(
      mode === "direct"
        ? `Intent drift high: ${reason}${driverText}`
        : `Intent drift is high: ${reason}${driverText}`,
    );
  }

  if (
    isLateNight(session.firstActivityAt || session.startedAt) &&
    session.distractionAverage > 1.4
  ) {
    messages.push(
      mode === "direct"
        ? "Late-night pull again."
        : "Late-night pull shows up again.",
    );
  }

  const previous = findPreviousSession(state, session);
  if (previous) {
    const currentTop = buildTopDomains(session)[0]?.domain;
    const prevTop = buildTopDomains(previous)[0]?.domain;
    if (currentTop && prevTop && currentTop === prevTop) {
      messages.push(
        mode === "direct"
          ? `Same top site: ${currentTop}.`
          : `Same top site as last session: ${currentTop}.`,
      );
    }
    const prevTrap = (previous.trapDoors || [])[0];
    if (trap && prevTrap) {
      const trapDomain = getDomain(trap.url);
      const prevTrapDomain = getDomain(prevTrap.url);
      if (trapDomain && prevTrapDomain && trapDomain === prevTrapDomain) {
        messages.push(
          mode === "direct"
            ? `Same turning point as last time: ${trapDomain}.`
            : `Same turning point as last session: ${trapDomain}.`,
        );
      }
    }
  }
...
function findPreviousSession(state, currentSession) {
  if (!state || !state.sessionOrder || !currentSession) {
    return null;
  }
...
function renderRankList(container, items, labelGetter, valueGetter = null) {
  if (!container) {
    return;
  }
  container.innerHTML = "";
  if (!items.length) {
    ...
    updateRankListVisibility(container, 0);
    return;
  }
  const fragment = document.createDocumentFragment();
  items.forEach((item, index) => {
    ...
    row.dataset.rankIndex = String(index + 1);
    const label = truncate(labelGetter(item), 32);
  ...
  container.appendChild(fragment);
  updateRankListVisibility(container, items.length);
}

function updateRankListVisibility(container, countOverride = null) {
  if (!container) {
    return;
  }
  const rawLimit = Number(container.dataset.limit || 0);
  ...
  const total = Number.isFinite(countOverride) ? countOverride : items.length;
  items.forEach((item, index) => {
    const hidden = collapsed && index + 1 > limit;
    item.classList.toggle("is-hidden", hidden);
  });
  const toggle =
    container.closest(".rank-block")?.querySelector(
      `.rank-toggle[data-target="${container.id}"]`,
    ) || null;
  if (!toggle) {
    return;
  }
  toggle.hidden = total <= limit;
...
function formatSessionLabel(session) {
  const timestamp =
    session.firstActivityAt || session.startedAt || session.updatedAt;
  return formatSessionDay(timestamp);
}
...
function getSessionDateKey(session) {
  return formatDateKey(
    session?.startedAt || session?.firstActivityAt || session?.updatedAt || 0,
  );
}

function findSessionByDateKey(key) {
  if (!key || !app.state?.sessionOrder) {
    return null;
  }
  const sessions = app.state.sessionOrder
    .map((id) => app.state.sessions[id])
  ...
  const matches = sessions.filter(
    (session) => getSessionDateKey(session) === key,
  );
  if (!matches.length) {
    return null;
  }
  return matches.sort((a, b) => (b.startedAt || 0) - (a.startedAt || 0))[0];
}
